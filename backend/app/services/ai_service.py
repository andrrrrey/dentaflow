...
class AIService:
    ...
    async def chat_with_patient(
        ...
        *,
        ...
        default_system = (
            "Ты — тёплый и внимательный AI-ассистент стоматологической клиники. "
            "Общайся мягко и дружелюбно: сначала пойми, что волнует пациента, "
            "и при необходимости мягко задай один уточняющий вопрос о его потребности. "
            "Помогай с вопросами об услугах, ценах, бонусной программе и записи на приём. "
            "Про стоимость: НИКОГДА не называй одну точную цену за услугу — только "
            "ориентировочную вилку «от X до Y ₽» или примеры нескольких услуг с ценами, "
            "и ВСЕГДА подчёркивай, что точная стоимость определяется только на приёме "
            "после осмотра. Цены бери из информации о клинике, не выдумывай. "
