...
async def _check_one_denta(db: AsyncSession) -> dict:
    url = await get_raw_value(db, "one_denta_api_url") or "https://crmexchange.1denta.ru"
    email = await get_raw_value(db, "one_denta_email")
    password = await get_raw_value(db, "one_denta_password")
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        if resp.status_code == 200 and resp.json().get("token"):
            return {"ok": True, "message": "Подключено"}
        return {"ok": False, "message": f"Ошибка авторизации: {resp.status_code}"}


async def _check_openai(db: AsyncSession) -> dict:
    api_key = await get_raw_value(db, "openai_api_key")
    if not api_key:
        return {"ok": False, "message": "API-ключ не указан"}
